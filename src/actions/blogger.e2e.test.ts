...
describe('Blogger API Service - E2E Live Tests with Output', () => {
  ...
  describe('getPostList', () => {
    it('should fetch a real list of posts and print the result', { timeout: E2E_TEST_TIMEOUT }, async () => {
      logSeparator();
    ...
    it('should fetch posts within a date range and print the result', { timeout: E2E_TEST_TIMEOUT }, async () => {
      logSeparator();
      const params = {
        'start-index': 1,
        'max-results': 3,
  ...
  describe('getPostContent', () => {
    it('should fetch the real content for a specific post and print the result', { timeout: E2E_TEST_TIMEOUT }, async () => {
  ...
  describe('getPageList', () => {
    it('should fetch a real list of pages and print the result', { timeout: E2E_TEST_TIMEOUT }, async () => {
      const params = { 'start-index': 1, 'max-results': 2 };
      logSeparator();
  ...
  describe('getPostListByCategories', () => {
    it('should fetch posts by category and print the result', { timeout: E2E_TEST_TIMEOUT }, async () => {
      ...
      expect(result.feed.entry.length).toBeGreaterThan(0);
      const hasCategory = result.feed.entry.some(post =>
        post.category?.some(cat => cat.term === category)
      );
  ...
  describe('getAllTags', () => {
    it('should efficiently fetch all blog tags and print the result', { timeout: E2E_TEST_TIMEOUT }, async () => {
      logSeparator();
